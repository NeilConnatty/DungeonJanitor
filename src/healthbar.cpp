...
bool HealthBar::validate_textures()
{
	if (!m_health_texture.is_valid())
	{
		if (!m_health_texture.load_from_file(textures_path("ui/ui_dungeon_health.png")))
		{
			fprintf(stderr, "Failed to load texture health_texture\n");
			return false;
		}
	}
	if (!m_bar_texture.is_valid())
	{
		if (!m_bar_texture.load_from_file(textures_path("ui/ui_dungeon_health_bar.png")))
		{
