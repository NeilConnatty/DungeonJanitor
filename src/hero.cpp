...
void Hero::update_path()
{
	...
	else if (!is_moving())
	{
		if (m_currentRoom->get_artifact()->is_activated())
		{
			...
			Pathfinder::getPathFromPositionToDestination(m_position, artifact_pos, 1.0f, 1.0f, //SPEED / 10.f, Y_SPEED / 10.f, 
		...
		else
		{
			...
			Pathfinder::getPathFromPositionToDestination(m_position, next_door_pos, 1.0f, 1.0f, //SPEED / 10.f, Y_SPEED / 10.f,
